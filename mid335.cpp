...
float arr[111] = {16.35, 17.32, 18.35, 19.45, 20.60, 21.83, 23.12, 24.50, 25.96, 27.50, 29.14, 30.87, 32.70, 34.65, 36.71, 38.89, 41.20,  
43.65,
46.25,
49.00,
51.91,
...
int main(int argc, char *argv[]) {
	...
	for (int j = octave_delta; j < 12; j++) {
        ...
        for (i; i < k; i++) {
            ...
            float diff = freq((note_t)m, j) - arr[i - 1];

	    cout << m << "    " << j << "    " << freq((note_t)m, j) << "  " << fabs(diff);
   ...
   /* 
        for (int i = 1; i < 13; i++) {
            	float diff = freq((note_t)i, octave_delta) - arr[i - 1];

	    cout << i << "    " << octave_delta << "    " << freq((note_t)i, octave_delta) << "  " << fabs(diff);
	    	    if ( fabs(diff) > tolerance ) {
			            cout << "<---- Bad\n";
                        count++;
            	    } else {
